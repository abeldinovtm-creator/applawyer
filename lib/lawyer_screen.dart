...
class _LawyerDashboardScreenState extends State<LawyerDashboardScreen>
  ...
  Future<void> _sendResponse(String caseId, Map<String, dynamic> caseItem) async {
    ...
    // Учёт бесплатных откликов ведём в фоне (не блокирует отклик) — задел
    // на будущую платную модель сверх бесплатной квоты, отдельно от комиссии
    // с завершённых дел.
    _supabase.rpc('consume_free_contact', params: {'p_lawyer_id': user.id}).catchError((_) => false);
