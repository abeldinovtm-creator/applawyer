-- Уведомления теперь хранят case_id/conversation_id, чтобы в приложении
-- при тапе на уведомление можно было сразу открыть нужный чат/дело,
-- а не просто показать текст.

ALTER TABLE notifications
  ADD COLUMN IF NOT EXISTS case_id UUID REFERENCES cases(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS conversation_id UUID REFERENCES conversations(id) ON DELETE CASCADE;

-- call_send_push получает 2 новых необязательных параметра — старую
-- 3-аргументную версию убираем, чтобы не плодить дублирующийся оверлоад
-- (все вызовы ниже переписаны под новую сигнатуру).
DROP FUNCTION IF EXISTS call_send_push(UUID, TEXT, TEXT);
...
DECLARE
  v_key TEXT;
BEGIN
  INSERT INTO notifications (recipient_id, title, body, case_id, conversation_id)
  VALUES (p_recipient_id, p_title, p_body, p_case_id, p_conversation_id);
...
-- Юрист откликнулся → уведомить клиента, привязано к новой беседе.
-- Сохраняет языковую логику из 20260706_notification_language.sql —
-- она была бы затёрта повторным CREATE OR REPLACE этой же функции.
...
-- Клиент принял/отклонил → уведомить юриста, привязано к той же беседе.
...
-- Новое сообщение → уведомить собеседника, привязано к беседе чата.
